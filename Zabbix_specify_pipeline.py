...
class TFModel(Model, TFParams,
  ...
  def _transform(self, dataset):
    ...
    tf_args = self.args.argv if self.args.argv else local_args
    #date_rdd_out = dataset.select(date_col).collect()
...
def _run_model_tf2(iterator, args, tf_args):
  ...
  for tensors in yield_batch(iterator, args.batch_size, len(input_tensor_names)):
    ...
    #logger.info("====PREDictions : {}".format(output_tensor_names))
    ...
    #python_outputs : list type
    python_outputs = [v.numpy().tolist() for k, v in outputs.items()]
    #logger.info("===python_outputs : {}".format(python_outputs))
    #logger.info("===python_outputs : {}".format(python_outputs))
    np_python_outputs = np.array(python_outputs)
    #logger.info("===python_outputs : {}".format(np_python_outputs.shape))
    np_python_outputs = np.reshape(np_python_outputs,(1,1))
    np_python_outputs = data_scaler.inverse_transform(np_python_outputs)
    np_python_outputs = np.reshape(np_python_outputs,(1,1,1))
    python_outputs = np_python_outputs.tolist()
...
def yield_batch(iterable, batch_size, num_tensors=1):
  ...
  for item in iterable:
    #print("YIELDBATCH_ITEM = {}".format(iterable))
    if item is None:
      break
